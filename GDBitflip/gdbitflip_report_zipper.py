...
When executed, it elaborates all the reports in ./report/ and 
...
def report_zipper():
    ...
    summary_folder = f"./exec_reports/SUMMARIES/"
    ...
    reports_filenames = glob("./exec_reports/GDBitflip_exec_report_s*.txt")
    ...
    summary_filename = f"./exec_reports/SUMMARIES/GDBitflip_SUMMARY_{timestamp}.txt"
...
#    remaining_time = int((12000 - injection_total_cnt)/200) * (5*60 + 49)
#    seconds = int(divmod(remaining_time, 60)[1])
#    hours = int(divmod(divmod(remaining_time, 60)[0],60)[0])
#    minutes = int(divmod(divmod(remaining_time, 60)[0],60)[1])
#    print(f"[INFO] Esimated remaining time to 12K: {hours} hours {minutes} minutes {seconds} seconds.\n")
